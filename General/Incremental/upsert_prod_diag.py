#!/usr/bin/env python3
"""
upsert_prod_diag.py — Batched INSERT into rgd_udm_staging.diagnosis from udm_staging.diagnosis

SQL equivalent:
    INSERT INTO rgd_udm_staging.diagnosis (... udm_active_flag ...)
    SELECT a.*, NULL AS icd10_desc_std, NULL AS icd9_desc_std,
           NULL AS diag_coding_system_std, NULL AS primary_diagnosis_flag_std,
           'Y' AS udm_active_flag, a.udm_unq_id, a.updated_datetime, a.updated_by
    FROM udm_staging.diagnosis a
    INNER JOIN (
        SELECT udm_unq_id, nd_extracted_date, created_datetime,
               ROW_NUMBER() OVER (
        ...
        AND a.created_datetime  = ranked.created_datetime
        AND ranked.rn = 1
    WHERE a.psid = {PSID};

The ROW_NUMBER() deduplication subquery is pre-materialized ONCE into a staging table.
Because the ORDER BY uses two columns (nd_extracted_date DESC, created_datetime DESC),
STAGING_RANKED stores all three join keys so the batch INSERT picks exactly rn=1 rows.

Change SOURCE_TABLE, DEST_TABLE, and PSID at the top to run for any table/psid.

Pre-materialized staging:
  staging.upsert_prod_diag_ranked_{PSID}  — (udm_unq_id, nd_extracted_date, created_datetime) for rn=1 rows
...
SOURCE_TABLE = "udm_staging.diagnosis"        # staging/delta table
DEST_TABLE   = "rgd_udm_staging.diagnosis"    # production table to insert into
PSID         = 11

# ─────────────────────────────────────────────────────────────────────
_SUFFIX = f"psid{PSID}"

STAGING_RANKED   = f"staging.upsert_prod_diag_ranked_n_{_SUFFIX}"   # ROW_NUMBER dedup result
STAGING_PK       = f"staging.upsert_prod_diag_pk_n_{_SUFFIX}"       # eligible source ndids
CHECKPOINT_TABLE = f"staging.etl_checkpoint_upsert_prod_diag_n_{_SUFFIX}"
CHECKPOINT_KEY   = f"upsert_prod_diag.{_SUFFIX}"
...
def build_batch_insert(pk_lo, pk_hi):
    ...
    Three-column JOIN (udm_unq_id + nd_extracted_date + created_datetime) handles
    the two-column ORDER BY tiebreaker from the original SQL.
    """
    return f"""
INSERT INTO {DEST_TABLE}
    (diag_id, ndid, eid, enc_date, encounter_end_date, diag_date,
     ...
     enc_date_proxy,
     icd10_desc_std, icd9_desc_std, diag_coding_system_std, primary_diagnosis_flag_std,
     udm_active_flag, udm_unq_id,
...
SELECT
    ...
    a.diag_code_stripped,
    a.primary_diagnosis_flag,
    a.parent_diagnosis_code,
    a.parent_diagnosis_desc,
    a.icd_codeset,
    a.icd_codeset_desc,
    a.icd_codeset_group,
    a.icd_codeset_system,
    a.snomed_code,
    a.diag_severity,
    a.diag_status,
    a.diag_end_date,
    a.provisional_diag_flag,
    a.differential_diag_flag,
    a.comments_notes,
    a.diag_risk,
    a.specify,
    a.nd_extracted_date,
...
INNER JOIN {STAGING_RANKED} r
    ...
    AND a.created_datetime  = r.created_datetime
WHERE a.psid       = {PSID}
...
def setup_tables():
    ...
    try:
        ...
        # Computes the single latest row per udm_unq_id for psid=PSID.
        # ORDER BY nd_extracted_date DESC, created_datetime DESC — both columns
        # stored so the batch JOIN can pin exactly the rn=1 row.
        ...
        # ndids will actually be inserted — used for batch boundaries.
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  Upsert Prod Diagnosis ETL — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
