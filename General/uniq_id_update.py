#!/usr/bin/env python3
"""
uniq_id_update.py — Batched UPDATE to populate udm_unq_id on rgd_udm_staging.vitals

SQL equivalent:
    UPDATE rgd_udm_staging.vitals
...
TARGET_TABLE = "rgd_udm_staging.vitals"
WHERE_FILTER = "psid = '11'"          # set to None to update all rows

# ─────────────────────────────────────────────────────────────────────
_TABLE_SUFFIX = TARGET_TABLE.replace(".", "_").replace("-", "_")

STAGING_PK       = f"staging.uniq_id_pk21_{_TABLE_SUFFIX}"
CHECKPOINT_TABLE = f"staging.etl_checkpoint_uniq_id21_{_TABLE_SUFFIX}"
CHECKPOINT_KEY   = f"uniq_id.vitals.pass1.{_TABLE_SUFFIX}"
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  udm_unq_id UPDATE — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
