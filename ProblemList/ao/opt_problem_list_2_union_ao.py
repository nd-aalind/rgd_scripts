#!/usr/bin/env python3
"""
Optimized Problem List ETL (v2) for: udm_staging.problemlist
...
Pre-materialized lookup tables (computed ONCE, reused across all batches):
  - staging.pl2_snomed_{SOURCE_SCHEMA}        (SNOMED where nd_active_flag='Y')
...
Usage:
    python opt_problem_list_2_union_ao.py
"""
...
SOURCE_SCHEMA = "raleigh"   # e.g. "tncpa", "raleigh", "tng_athena_one", ...
PSID          = 5

DEST_TABLE       = "udm_staging.problemlist_updated_ao"
CHECKPOINT_TABLE = f"staging.etl_checkpoint_pl2_athenaone__v6_{SOURCE_SCHEMA}"

# ── Pre-materialized lookup staging tables ───────────────────────────
STAGING_SNOMED = f"staging.pl2_snomed__v3_{SOURCE_SCHEMA}"
...
SOURCES = [
    {
        ...
        "pk":      "PATIENTPROBLEMID",
        "staging": f"staging.tmp_pl4_pp_staging_{SOURCE_SCHEMA}",
    },
    {
        "table":   "PATIENTSNOMEDPROBLEM",
        "pk":      "PATIENTSNOMEDPROBLEMID",
        "staging": f"staging.tmp_pl4_psp_staging_{SOURCE_SCHEMA}",
    },
]
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  AthenaOne Problem List (v2) ETL — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
