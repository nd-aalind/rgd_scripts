#!/usr/bin/env python3
"""
Optimized ETL loader for: udm_staging.problemlist (eCW source)
...
Pre-materialized lookup tables (computed ONCE, reused across all batches):
  - staging.pl_ecw_enc_{SOURCE_SCHEMA}         (enc table)
  - staging.pl_ecw_icdsynonyms_{SOURCE_SCHEMA} (icd_synonyms table)
...
Usage:
    python opt_problem_list_ecw.py
"""
...
SOURCE_SCHEMA = "ecw"   # e.g. "ecw", "ecw_raleigh", ...
PSID          = 3

DEST_TABLE       = "udm_staging.problemlist"
STAGING_TABLE    = f"staging.tmp_pl_ecw_staging_{SOURCE_SCHEMA}"
CHECKPOINT_TABLE = f"staging.etl_checkpoint_pl_ecw_{SOURCE_SCHEMA}"
CHECKPOINT_KEY   = f"problemlist.insert.{SOURCE_SCHEMA}"

BATCH_KEY = "SlNo"

# ── Pre-materialized lookup staging tables ───────────────────────────
STAGING_ENC         = f"staging.pl_ecw_enc_{SOURCE_SCHEMA}"
STAGING_ICDSYNONYMS = f"staging.pl_ecw_icdsynonyms_{SOURCE_SCHEMA}"
...
SELECT
    ...
    CAST({p}.SNOMEDDesc AS CHAR),
    CAST({p}.SNOMED AS CHAR(50)),
    CAST({icd}.ICD9_SNOMEDCT_IMO AS CHAR(50)),
    CAST({p}.problemtype AS CHAR(100)),
...
LEFT JOIN {STAGING_ENC} {enc}         ON {enc}.encounterid = {p}.encounterID
LEFT JOIN {STAGING_ICDSYNONYMS} {icd} ON {p}.synonymid = {icd}.id
...
def setup_tables():
    ...
    print("  Materializing icd_synonyms lookup...")
    if not _table_exists(cur, STAGING_ICDSYNONYMS):
        cur.execute(f"""
            CREATE TABLE {STAGING_ICDSYNONYMS} AS
            SELECT * FROM {SOURCE_SCHEMA}.icd_synonyms
        """)
        cur.execute(f"ALTER TABLE {STAGING_ICDSYNONYMS} ADD INDEX idx_icd (id)")
        conn.commit()
        print("    created")
    else:
        print("    already exists, reusing")
    cur.execute(f"SELECT COUNT(*) FROM {STAGING_ICDSYNONYMS}")
    print(f"    {cur.fetchone()[0]:,} icd_synonyms rows")
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  eCW Problem List ETL — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print(f"    DROP TABLE IF EXISTS {STAGING_ICDSYNONYMS};")
