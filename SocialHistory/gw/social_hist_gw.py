#!/usr/bin/env python3
"""
Optimized ETL loader for: udm_staging.socialhistory_new
Source: Greenway (GW)
...
Pre-materialized lookup table (computed ONCE, reused across all batches):
  - staging.sh_gw_phcd_v1_{schema}  — PatHistCatPatHistItem INNER JOIN PatHistCat (both active),
                                      stores PatHistCatPatHistItemID, PatHistCatDescription
                                      Keyed on PatHistCatPatHistItemID — both JOINs collapsed to one.

Batch key: PatHistSocialID (integer PK)

Column mapping to shared dest table (same structure as socialhistory_new / AO):
  hist_category   ('SocialHistory')         → social_hist_category
  social_category (PatHistCatDescription)   → social_hist_question
  social_option   (NULL)                    → social_hist_value
  social_notes    (PHSNote)                 → social_hist_notes

Optimizations:
- Both JOINs pre-collapsed into one staging table (not re-scanned per batch)
- Batching by actual PatHistSocialID values (sparse ID safe)
...
STAGING_PHCD     = f"staging.sh_gw_phcd_v1_{SOURCE_SCHEMA}"
STAGING_PK       = f"staging.tmp_sh_gw_new_v1_{SOURCE_SCHEMA}"
CHECKPOINT_TABLE = f"staging.etl_checkpoint_sh_gw_new_v1_{SOURCE_SCHEMA}"
CHECKPOINT_KEY   = f"pathhistsocial.gw.insert.{SOURCE_SCHEMA}"

BATCH_KEY = "PatHistSocialID"


# ── Date CASE helper (LEFT approach — matches AO social history) ──────────────
...
SELECT
    ...
    {PSID},
    NULL
FROM {SOURCE_SCHEMA}.PatHistSocial a
...
def setup_tables():
    ...
    _ensure_index(cur, conn, f"{SOURCE_SCHEMA}.PatHistSocial",
    ...
    # ── 3. Destination table (same structure as socialhistory_new / AO) ──
    ...
    # ── 5. PK staging ─────────────────────────────────────────────────
    print("  Creating PK staging for PatHistSocial...")
    if not _table_exists(cur, STAGING_PK):
        cur.execute(f"""
            CREATE TABLE {STAGING_PK} AS
            SELECT {BATCH_KEY}
            FROM {SOURCE_SCHEMA}.PatHistSocial
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  Greenway Social History ETL — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  source     : {SOURCE_SCHEMA}.PatHistSocial  (psid={PSID})")
    ...
    if not ranges:
        print(f"\nNo eligible rows in {SOURCE_SCHEMA}.PatHistSocial. Exiting.")
        return

    result = None
    with tqdm(total=len(ranges), desc="PatHistSocial", unit="batch") as pbar:
    ...
    if result["status"] == "done":
        ...
        tag = " FAIL"
    print(f"  [{tag}] {SOURCE_SCHEMA}.PatHistSocial  "
