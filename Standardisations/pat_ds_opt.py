#!/usr/bin/env python3
"""
Optimized deceased status standardisation UPDATE for: rgd_udm_silver.patients

No lookup table join — pure CASE expression on pat_deceased_status and deceased_date.

Updates one column:
  - pat_deceased_status_std  (standardised deceased status label)

Mapping:
  pat_deceased_status IN ('1','Y') OR deceased_date IS NOT NULL → 'Deceased'
  pat_deceased_status = '0'                                     → 'Null'
  pat_deceased_status IS NULL                                   → 'Null'
  ELSE                                                          → 'Flagged'
...
STAGING_TABLE    = "staging.tmp_pat_ds_std_staging"
CHECKPOINT_TABLE = "staging.etl_checkpoint_pat_ds_std"

# ── Primary key column used for batching ─────────────────────────────
BATCH_KEY = "udm_inc_id"

# Checkpoint key — single entry since this is one UPDATE operation
CHECKPOINT_KEY = "patients.pat_deceased_status_std_update"
...
def build_batch_update(pk_lo, pk_hi):
    ...
    All CASE expressions are preserved exactly.
    """
    return f"""
UPDATE {TARGET_TABLE}
...
def run_update(ranges, pbar):
    """Execute the deceased status standardisation UPDATE across all batches."""
    conn = get_connection()
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  Deceased Status Standardisation UPDATE — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  target     : {TARGET_TABLE}")
    print(f"  column     : pat_deceased_status_std")
