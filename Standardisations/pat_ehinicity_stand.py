#!/usr/bin/env python3
"""
Optimized ethnicity standardisation UPDATE for: rgd_udm_silver.patients

Joins to semantics.ethnicity lookup table (twice — by code, then by name)
and updates two columns:
  - pat_ethnicity_code_std   (standardised ethnicity code)
  - pat_ethnicity_std        (standardised ethnicity label)
...
STAGING_TABLE    = "staging.tmp_ethnicity_std_staging"
CHECKPOINT_TABLE = "staging.etl_checkpoint_ethnicity_std"

# ── Primary key column used for batching ─────────────────────────────
BATCH_KEY = "udm_inc_id"

# Checkpoint key — single entry since this is one UPDATE operation
CHECKPOINT_KEY = "patients.ethnicity_std_update"
...
def build_batch_update(pk_lo, pk_hi):
    ...
    Two LEFT JOINs to semantics.ethnicity:
      b1 — match by code  (a.pat_ethnicity_code = b1.Code)
      b2 — match by name  (LOWER(a.pat_ethnicity) = LOWER(b2.ethnicity))
    b1 takes priority over b2 in the CASE expressions.
    """
    return f"""
UPDATE {TARGET_TABLE} a
LEFT JOIN semantics.ethnicity b1 ON a.pat_ethnicity_code = b1.Code
LEFT JOIN semantics.ethnicity b2 ON LOWER(a.pat_ethnicity) = LOWER(b2.ethnicity)
SET
...
def run_update(ranges, pbar):
    """Execute the ethnicity standardisation UPDATE across all batches."""
    conn = get_connection()
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  Ethnicity Standardisation UPDATE — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  target     : {TARGET_TABLE}")
    print(f"  lookup     : semantics.ethnicity (joined twice — by code and by name)")
