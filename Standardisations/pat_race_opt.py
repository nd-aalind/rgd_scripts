#!/usr/bin/env python3
"""
Optimized race standardisation UPDATE for: rgd_udm_silver.patients

Joins to semantics.race lookup table (twice — by code, then by name)
...
STAGING_TABLE    = "staging.tmp_race_std_staging"
CHECKPOINT_TABLE = "staging.etl_checkpoint_race_std"

# ── Primary key column used for batching ─────────────────────────────
BATCH_KEY = "udm_inc_id"

# Checkpoint key — single entry since this is one UPDATE operation
CHECKPOINT_KEY = "patients.race_std_update"
...
def build_batch_update(pk_lo, pk_hi):
    ...
    Two LEFT JOINs to semantics.race:
      b1 — match by code  (a.pat_race_code = b1.Code)
      b2 — match by name  (LOWER(a.pat_race) = LOWER(b2.race))
...
def run_update(ranges, pbar):
    """Execute the race standardisation UPDATE across all batches."""
    conn = get_connection()
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  Race Standardisation UPDATE — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  target     : {TARGET_TABLE}")
    print(f"  lookup     : semantics.race (joined twice — by code and by name)")
