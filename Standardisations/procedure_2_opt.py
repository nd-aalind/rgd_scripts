#!/usr/bin/env python3
"""
Optimized procedures standardisation pass-2 UPDATE for: rgd_udm_silver.procedures

Second-pass cleanup that maps rows where proc_code IS NULL but proc_name IS NOT NULL,
using LIKE-pattern CASE expressions directly on proc_name — no lookup table join.

Updates only rows where:
    proc_code IS NULL AND proc_name IS NOT NULL
...
- Staging table pre-filters to eligible rows only (no wasted batches)
...
STAGING_TABLE    = "staging.tmp_procedures_std2_staging"
CHECKPOINT_TABLE = "staging.etl_checkpoint_procedures_std2"

# ── Primary key column used for batching ─────────────────────────────
BATCH_KEY = "udm_inc_id"

# Checkpoint key — single entry since this is one UPDATE operation
CHECKPOINT_KEY = "procedures.proc_code_std2_update"
...
def build_batch_update(pk_lo, pk_hi):
    ...
    Original WHERE filter (proc_code IS NULL AND proc_name IS NOT NULL)
    is combined with the batch range filter.
    All CASE/LIKE expressions are preserved exactly.
    """
    return f"""
UPDATE {TARGET_TABLE}
SET
...
def setup_tables():
    ...
    # ── 1. Staging table: only eligible rows — no wasted batches ──────
    print("  Creating staging table (proc_code IS NULL AND proc_name IS NOT NULL rows)...")
    cur.execute(
...
def run_update(ranges, pbar):
    """Execute the pass-2 procedures standardisation UPDATE across all batches."""
    conn = get_connection()
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  Procedures Standardisation Pass 2 UPDATE — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  target     : {TARGET_TABLE}")
    print(f"  filter     : proc_code IS NULL AND proc_name IS NOT NULL")
    ...
    ranges = setup_tables()
    if not ranges:
        print(f"\nNo eligible rows found in {TARGET_TABLE}. Nothing to update.")
        return
